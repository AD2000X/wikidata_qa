...
class WikidataQA:
    ...
    def ask(self, question: str, endpoint: str = WIKIDATA_SPARQL) -> Dict[str, Any]:
        ...
        entity_name = extract_entity_name(question, intent)
        candidate = self.resolver.resolve(entity_name, intent=intent, question=question)
        sparql = build_sparql(candidate.qid, intent)
        ttl = INTENT_CONFIG[intent].ttl_seconds

        rows = execute_sparql(sparql, cache=self.cache, ttl_seconds=ttl, endpoint_url=endpoint)
        if not rows:
            raise SPARQLExecutionError(f"No answer returned for question: {question}")
    ...
    async def ask_async(
        ...
        intent = detect_intent(question)
        entity_name = extract_entity_name(question, intent)
        candidate = await self.resolver.resolve_async(entity_name, intent, question, client, search_semaphore, sparql_semaphore)
        sparql = build_sparql(candidate.qid, intent)
        ttl = INTENT_CONFIG[intent].ttl_seconds

        rows = await execute_sparql_async(sparql, client, sparql_semaphore, cache=self.cache, ttl_seconds=ttl)
        if not rows:
            raise SPARQLExecutionError(f"No answer returned for question: {question}")
...
async def ask_batch(
    ...
    *,
    ...
    async with httpx.AsyncClient(limits=limits, headers={"User-Agent": USER_AGENT}) as client:
        tasks = [qa.ask_async(q, client, search_semaphore, sparql_semaphore) for q in questions]
        return await asyncio.gather(*tasks)


def ask(question: str, endpoint: str = WIKIDATA_SPARQL) -> str:
