...
All constants, QID mappings, and intent definitions live here.
FALLBACK_QIDS is loaded from qid_fallback.json at import time;
if the file is missing or malformed the process fails immediately.
"""
...
FALLBACK_QID_PATH = BASE_DIR / "qid_fallback.json"
...
USER_AGENT = f"Wikidata-Ask-Bot/7.0 ({sys.platform}) Python/{sys.version_info[0]}.{sys.version_info[1]}"
DEFAULT_TIMEOUT = 10
DEFAULT_LANGUAGE = "en"
...
# =========================================================
# Disambiguation overrides (code-level, rarely changes)
# =========================================================

DISAMBIGUATION_RULES: Dict[str, str] = {
    "madonna": "Q1744",
    "new york": "Q60",
    "new york city": "Q60",
    "london": "Q84",
...
# =========================================================
# Fallback QID map (loaded from external JSON)
#
# Ops can edit qid_fallback.json to add new entities
# without touching Python code. If the file is missing
# or contains invalid JSON, the process fails immediately.
# =========================================================

if not FALLBACK_QID_PATH.exists():
    raise FileNotFoundError(
        f"QID fallback file not found: {FALLBACK_QID_PATH}\n"
        f"This file is required for entity resolution fallback. "
        f"Place a valid JSON file at the path above."
    )

FALLBACK_QIDS: Dict[str, Dict[str, str]] = json.loads(
    FALLBACK_QID_PATH.read_text(encoding="utf-8")
)
